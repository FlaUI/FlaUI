...
namespace FlaUI.Core.UITests.Elements
{
    ...
    public class MenuTests : UITestBase
    {
        ...
        public void TestMenuWithSubMenus()
        {
            ...
            Assert.That(items[0].Information.Name, Is.EqualTo("File"));
            Assert.That(items[1].Information.Name, Is.EqualTo("Edit"));
            var subitems1 = items[0].SubMenuItems;
            Assert.That(subitems1, Has.Length.EqualTo(1));
            Assert.That(subitems1[0].Information.Name, Is.EqualTo("Exit"));
            var subitems2 = items[1].SubMenuItems;
            Assert.That(subitems2, Has.Length.EqualTo(2));
            Assert.That(subitems2[0].Information.Name, Is.EqualTo("Copy"));
            Assert.That(subitems2[1].Information.Name, Is.EqualTo("Paste"));
            var subsubitems1 = subitems2[0].SubMenuItems;
            Assert.That(subsubitems1, Has.Length.EqualTo(2));
            Assert.That(subsubitems1[0].Information.Name, Is.EqualTo("Plain"));
            Assert.That(subsubitems1[1].Information.Name, Is.EqualTo("Fancy"));
        }
    }
}
