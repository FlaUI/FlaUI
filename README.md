...
- UIA3: Com Library for UI Automation
  - UIA3 is the newest of them all which is still the current version (and should be maintained). This one works great for WPF / Windows Store Apps but unfortunately, it can have some bugs with WinForm applications (see [FAQ](https://github.com/Roemer/FlaUI/wiki/FAQ)) which are not existent in UIA2.
